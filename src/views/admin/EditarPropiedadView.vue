<script setup>
    ...
    import { doc, updateDoc } from 'firebase/firestore' // Importar la función updateDoc de la librería de Firestore que permite actualizar un documento
    ...
    const docdRef = doc(db, 'propiedades', route.params.id) // Referencia al documento de la propiedad a editar
    const propiedad = useDocument(docdRef) // Obtener la propiedad a editar
    ...
    const submit = handleSubmit( async values => {
        const { imagen, ...propiedad } = values // Extraer la imagen del objeto propiedad
        ...
        if(image.value){
             ...
             await updateDoc(docdRef, data)
        }else{
             const data = {
                 ...propiedad,
                 ubicacion: center.value
             }
             await updateDoc(docdRef, data) // Actualizar el documento de la propiedad
        }

        router.push({name: 'admin-propiedades'}) // Redirigir al panel de administración
    })
